...
| [scripts/claude-local.sh](scripts/claude-local.sh) | Launch Claude Code against local model (restores config on exit) |
...
### 3. Open SSH tunnel from your machine
...
- `CLAUDE_CODE_USE_BEDROCK` in `settings.json` overrides shell env vars — use `claude-local.sh` to swap configs safely
